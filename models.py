...
from tensorflow.keras.layers import Input, LSTM, LSTMCell, Dense, Conv1D, Conv1DTranspose, Concatenate, Cropping1D, LayerNormalization, Dropout
...
def Unet1D_v4():
    ...
    dc6 = Conv1DTranspose(1,32,2,padding='same')(conc)
    conc = Concatenate()([inp,dc6])
    dc7 = Conv1DTranspose(1,32,1,padding='same',activation='relu')(conc)
    drop_out = Dropout(rate = 0.3)(dc7)
    output = Conv1DTranspose(1, 64, 2, padding = 'same', activation = 'tanh')(drop_out)
    model = keras.models.Model(inp,output)
    return model
